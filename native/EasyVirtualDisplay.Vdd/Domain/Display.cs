...
internal class Display
{
    ...
    public class ModeSet
    {
        public int Width;
        public int Height;
        public List<int> RefreshRates = new();
    }
    ...
    public string AdapterInstance = string.Empty;
    ...
    public override string ToString()
    {
        ...
        if (CloneOf > 0 && CloneOf < Identifier)
        {
            str += $" (clone of [{CloneOf}])";
        }
    ...
    private void FetchAllModes()
    {
        ...
        for (int num = -1; Native.EnumDisplaySettings(DeviceName, num, ref devMode); num++)
        {
            ...
            else
            {
                ...
                mode.Hz = 0;
                if (set.TryGetValue(mode.Bits, out var refreshRates))
                {
        ...
        foreach (var kv in set)
        {
            ...
            var refreshRates = kv.Value.ToList();
    ...
    public bool ChangeMode(int? width, int? height, int? hz, Orientation? orientation)
    {
        ...
        if (Native.EnumDisplaySettings(DeviceName, -1, ref mode))
        {
            if (width.HasValue)
            {
                ...
                mode.dmFields |= 0x80000;
            }

            if (height.HasValue)
            {
                mode.dmPelsHeight = height.Value;
                mode.dmFields |= 0x100000;
            }
            ...
            {
                var newDisplayOrientation = orientation.Value;
                mode.dmDisplayOrientation = newDisplayOrientation;

                if (((int)newDisplayOrientation + (int)CurrentOrientation) % 2 != 0)
                {
                    int swapped = mode.dmPelsWidth;
                    mode.dmPelsWidth = mode.dmPelsHeight;
                    mode.dmPelsHeight = swapped;
                }
            }

            return Native.ChangeDisplaySettingsEx(DeviceName,
                ref mode, IntPtr.Zero, 0x1 | 0, IntPtr.Zero) == 0;
        }
    ...
    {
        return string.Join(",", modes.Select(mode => mode.Bits.ToString("x")));
    }
    ...
    public static List<Mode> ParseModes(string modes)
    {
        ...
        {
            if (ulong.TryParse(mode, NumberStyles.HexNumber, CultureInfo.InvariantCulture, out var bits))
            {
    ...
    public static List<Display> GetAllDisplays()
    {
        ...
        for (int i = 0; Native.EnumDisplayDevices(null, i, ref dd, 0); i++)
        {
            ...
            for (int j = 0; Native.EnumDisplayDevices(dd.DeviceName, j, ref dd2, Native.EDD_GET_DEVICE_INTERFACE_NAME); j++)
            {
                ...
                var pathIdx = paths.FindIndex(path => dd2.DeviceID.Contains(path.Replace('\\', '#')));
                if (pathIdx < 0)
                {
                    continue;
                }
                ...
                if (!displayMap.ContainsKey(paths[pathIdx]))
                {
                    ...
                    if (Device.GetDeviceInstance(paths[pathIdx], out uint devInst))
                    {
                        display.LastArrival = Device.GetDeviceLastArrival(devInst);

                        if (Device.GetParentDeviceInstance(devInst, out uint parentInst, out display.AdapterInstance))
                        {
    ...
    private static List<string> GetDisplayPaths()
    {
        ...
        using (var key = Registry.LocalMachine.OpenSubKey(@"SYSTEM\CurrentControlSet\Services\monitor\Enum", false))
        {
            if (key != null)
            {
                ...
                {
                    var path = Convert.ToString(key.GetValue($"{i}"));
    ...
    private static int ParseDisplayAddress(string path)
    {
    ...
    private static string ParseDisplayCode(string id)
    {
        var tokens = id.Split('#');
